...
Variables
     ...
     b(i,j)   'Binary Constraint for getting ALL or NONE operation in x(i,j)' ;
...
Equation
   ...
   binaryconstraint(i,j) 'Adding binary constraint for ALL or NONE operation'
    binaryaddition(i) 'Adding binary variables for each Area should give one';
...
binaryconstraint(i,j).. x(i,j) =e= n(i)*b(i,j);

binaryaddition(i).. sum(j, b(i,j)) =e= 1;


Model schooltransport /all/ ;

Solve schooltransport using MIP minimizing z ;
